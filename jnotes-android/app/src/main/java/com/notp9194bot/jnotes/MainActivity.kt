...
import com.notp9194bot.jnotes.data.remote.ConfigSync
...
class MainActivity : FragmentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Draw under the status / nav bars edge-to-edge AND extend into
        // any display cutout, then hide the system bars so the note app
        // takes the entire screen. Bars reappear with a swipe from the
        // edge (BEHAVIOR_SHOW_TRANSIENT_BARS_BY_SWIPE) and auto-hide.
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            // R+ supports LAYOUT_IN_DISPLAY_CUTOUT_MODE_ALWAYS for true edge-to-edge.
            window.attributes = window.attributes.apply {
                @Suppress("InlinedApi")
                layoutInDisplayCutoutMode =
        ...
        // Seed the local display name with the device IP on first launch and
        // pull the latest server URL the admin has broadcast. Then re-poll
        // periodically so push-from-admin propagates without a restart.
        ioScope.launch {
            ConfigSync.bootstrap(this@MainActivity)
            while (true) {
                delay(60_000L)
                runCatching { ConfigSync.pullServerUrl(this@MainActivity) }
            }
        }
