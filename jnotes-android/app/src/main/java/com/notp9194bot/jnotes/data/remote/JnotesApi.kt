...
class JnotesApi(private val baseUrl: String, private val adminToken: String? = null) {
    ...
    /** Reads the broadcast server URL the admin has set on the relay. */
    ...
    /** Per-user moderation flags pushed by the admin. */
    data class BlockState(val blockChat: Boolean, val blockFeedback: Boolean)

    suspend fun fetchBlockState(userId: String): BlockState? = withContext(Dispatchers.IO) {
        runCatching {
            val (code, body) = request("GET", "/api/users/$userId/block", null)
            if (code !in 200..299) return@withContext null
