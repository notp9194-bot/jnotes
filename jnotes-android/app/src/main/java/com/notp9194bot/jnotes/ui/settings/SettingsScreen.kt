...
fun SettingsScreen(onBack: () -> Unit) {
    ...
    Scaffold(
        ...
        Column(
            ...
            SectionCard(title = "Feedback & chat server") {
                Text(
                    "Configure the relay server that delivers your feedback and chat messages to the admin.",
                    style = MaterialTheme.typography.bodySmall,
                    color = MaterialTheme.colorScheme.onSurface.copy(alpha = 0.7f),
                )
                ...
                var displayName by remember(s.displayName) { mutableStateOf(s.displayName) }
                OutlinedTextField(
                    value = displayName,
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    Button(onClick = {
                        ...
                        scope.launch { snackbar.showSnackbar("Server settings saved") }
                    }) { Text("Save") }
                    if (s.serverUrl.isNotBlank()) {
                        OutlinedButton(onClick = {
                            serverUrl = ""
                            vm.setServerUrl("")
                            scope.launch { snackbar.showSnackbar("Server URL cleared") }
                        }) { Text("Clear") }
                    }
                }
            }
            ...
            SectionCard(title = "About") {
                ...
                Text("Fully offline notes app built with Kotlin + Jetpack Compose.", style = MaterialTheme.typography.bodyMedium)
            }
        }
    }
